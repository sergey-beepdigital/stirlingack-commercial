...
<div class="images">
    ...
    <?php
        if ( isset($images) && is_array($images) && !empty($images) ) {
            ...
            {
                echo '<li>' . apply_filters( 'propertyhive_single_property_image_html', sprintf( '<a href="%s" class="propertyhive-main-image" title="%s" data-fancybox="gallery"><img class="lazy" data-src="%s"></a>', esc_attr( $image['url'] ), esc_attr( $image['title'] ), $image['url'] ), $post->ID ) . '</li>';
            }
    ...
    <?php if(sizeof($images) > 0) { ?>
        ...
        <div class="images-count">
            <i class="fa-light fa-camera"></i> <?php echo sizeof($images); ?>
        </div>
